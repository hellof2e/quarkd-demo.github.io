import{c as a,o as n,a as t}from"./vue.67f2eff0.js";const l={class:"markdown-body"},e=t(`<h1>Quick Start</h1><h2>Preview</h2><blockquote><p>Scan the QR code below to experience the Quark component library example</p></blockquote><img src="https://m.hellobike.com/resource/helloyun/16682/76s6X_quark.demo.png?x-oss-process=image/quality,q_80" width="200" alt="qr.png"><h2>CDN</h2><p>The easiest way to use quark is to directly introduce the CDN link in the HTML file, and then you can use it anywhere in the world. Since quark has achieved CSS-IN-JS, you only need to load the link below.</p><pre><code>&lt;!-- Introduce CDN files, only 80kb --&gt;
...
npm i quarkd
</code></pre><p><strong>React usage notes</strong>: Since the components provided by <code>quarkd</code> are all native custom elements (analogous to div), events dispatched in the component need to be received by <code>addEventLisener</code>, such as the custom close event <code>close</code> inside the <code>dialog</code> component. The Vue technology stack can directly use <code>@xx</code> to receive natively dispatched events, so there is no need to use <code>addEventLisener</code> to receive. <br><br> In order to improve the development experience, we have <a href="https://github.com/BBKolton/reactify-wc/">Reactify (Reactify)</a> for <code>quarkd</code>! So, we recommend that you use <code>@quarkd/quark-react</code> in your React/Preact projects!</p><pre><code class="language-bash"><span class="hljs-comment"># Recommended Use for React</span>
...
</code></pre><h3>Angular</h3><pre><code>import { Component } from &#39;@angular/core
...
</code></pre><h2>3 \u3001Polyfill</h2><blockquote><p>Use Quark(Web components) today and have them work in all major browsers. <a href="https://www.webcomponents.org/polyfills">Polyfill</a>.</p></blockquote><p>Install polyfills</p><pre><code>npm install @webcomponents/webcomponentsjs
</code></pre><p>You can also load the code from a CDN such as unpkg: <a href="https://unpkg.com/@webcomponents/webcomponentsjs@%5E2/">https://unpkg.com/@webcomponents/webcomponentsjs@^2/</a></p>`,25),p=[e],i={__name:"quickstart.en-US",setup(c,{expose:s}){return s({frontmatter:{}}),(r,h)=>(n(),a("div",l,p))}};export{i as default};
